...
export const updateDriverLocation = async (req, res) => {
  try {
    ...
    const updatedDelivery = await deliveryService.updateDriverLocation(
      id,
      currentLocation
    );
    res.status(200).json(updatedDelivery);
...
export const verifyDeliveryCode = async (req, res) => {
  try {
    ...
    const verifiedDelivery = await deliveryService.verifyDeliveryCode(
      orderId,
      verificationCode
    );
...
export const getDeliveriesByCustomer = async (req, res) => {
  try {
    ...
    const deliveries = await deliveryService.getDeliveriesByCustomer(
      customerId
    );
    res.status(200).json(deliveries);
  } catch (error) {
...
export const getDeliveriesByRestaurant = async (req, res) => {
  try {
    ...
    const deliveries = await deliveryService.getDeliveriesByRestaurant(
      restaurantId
    );
    res.status(200).json(deliveries);
  } catch (error) {
