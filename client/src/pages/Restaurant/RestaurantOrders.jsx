...
const RestaurantOrders = ({ restaurantId }) => {
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    try {
      ...
      fetchOrders(); // Refresh after updating
    } catch (err) {
          ...
          {orders.map((order) => (
            <div key={order._id} className="border p-4 rounded shadow bg-white">
              ...
              <p>
                <strong>Items:</strong>{" "}
                {order.items
                  .map((i) => `${i.menuItem.data.menuItemName} x ${i.quantity}`)
                  .join(", ")}
              </p>
              <p>
              ...
              <div className="space-x-2">
                {!order.isOrderAccepted ? (
                  ...
                  <p className="text-green-600 font-semibold">Order Accepted</p>
                )}
              </div>
              {order.isOrderAccepted && (
                <div className="space-x-2">
                  ...
                  >
                    ...
                    <option value="Picked Up" disabled>Picked Up</option>
                ...
                <div className="relative">
                  ...
                  <textarea
                    ...
                    onChange={async (e) => {
                      ...
                      // Call only if the code is 6 digits long
                      if (newCode.length === 6) {
